...
If `docgraph serve` is not running, the plugin can install, but Claude Code or Codex cannot initialize the HTTP MCP connection. DocGraph tools may be absent from the session, or calls may fail with a local connection error. Start the server and open a new agent session.
